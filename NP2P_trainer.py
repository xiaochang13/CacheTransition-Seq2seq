...
def main(_):
    ...
    if FLAGS.infile_format == 'fof':
        trainset = NP2P_data_stream.read_generation_datasets_from_fof(FLAGS.train_path, isLower=FLAGS.isLower)
    else:
        trainset = NP2P_data_stream.read_all_GenerationDatasets(FLAGS.train_path, isLower=FLAGS.isLower)
    print('Number of training samples: {}'.format(len(trainset)))

    FLAGS.feat_num = len(trainset[0][3][0])
    print('Number of features: {}'.format(FLAGS.feat_num))

    print('Loading test set.')
    if FLAGS.infile_format == 'fof':
        testset = NP2P_data_stream.read_generation_datasets_from_fof(FLAGS.test_path, isLower=FLAGS.isLower)
    else:
        testset = NP2P_data_stream.read_all_GenerationDatasets(FLAGS.test_path, isLower=FLAGS.isLower)
    ...
    trainDataStream = NP2P_data_stream.DataStream(trainset, word_vocab, char_vocab, POS_vocab, feat_vocab, action_vocab,
            options=FLAGS, isShuffle=True, isLoop=True, isSort=True)

    devDataStream = NP2P_data_stream.DataStream(testset, word_vocab, char_vocab, POS_vocab, feat_vocab, action_vocab,
