...
class Hypothesis(object):
    ...
    def extractFeatures(self, uniform=False):
        ...
        return self.trans_state.extractFeatures(self.trans_state.phase, word_idx, concept_idx, self.cache_idx, uniform_arc=uniform)
...
def run_beam_search(sess, trans_system, model, feat_vocab, action_vocab, batch, cache_size, options):
    ...
    (initial_state, input_hiddens, input_mask,
                concept_hiddens, concept_mask) = model.run_encoder(sess, batch, options)
    ...
    results = [] # this will contain finished hypotheses (those that have emitted the </s> action)
    steps = 0
    ...
    while steps < options.max_answer_len and len(results) < options.beam_size:
        ...
        cur_wid_t = [] # [batch_size]
        cur_cid_t = [] # [batch_size]
        ...
        for h in hyps:
            ...
            # print('!!! extracted feat number {}'.format(len(feats)))
            ...
            cur_wid_t.append(h.word_focus)
            cur_cid_t.append(h.concept_focus)
        ...
        cur_wid_t = np.array(cur_wid_t, dtype='int32')
        cur_cid_t = np.array(cur_cid_t, dtype='int32')
        ...
        feed_dict[model.wid_t] = cur_wid_t
        feed_dict[model.cid_t] = cur_cid_t
        ...
        for i in range(devDataStream.get_num_batch()):
            ...
            for j in xrange(len(hyps)):
                ...
                if j == 0:
                    ...
                    print (curr_repr)
                    print (hyp.trans_state.toString().encode('utf-8'))
