...
def run_perceptron(args):
    ...
    def buildChoices(X, X_t, labelIndexer, table_dir):
        ...
        n_instances = len(X)
        for sent_index in xrange(n_instances):
    ...
    def saveIndexer(indexer, file):
        with open(file, 'w') as wf:
            print >>wf, len(indexer)
            for l in indexer:
                print >>wf, ("%s %d" % (l, indexer[l]))
            wf.close()
    ...
    def saveEmissionWeights(emissionWeights, featIndexer, labelIndexer, file):
        ...
        with open(file, 'w') as wf:
            print >>wf, observe_length, target_length
            for x_id in xrange(observe_length):
                for y_id in emissionWeights[x_id]:
                    if emissionWeights[x_id][y_id] != 0:
                        print >>wf, x_id, y_id, emissionWeights[x_id][y_id]
            wf.close()
    ...
    labelList = [None for x in xrange(num_labels+1)]
    ...
    for iter in xrange(args.iters):
        ...
        for curr_index in xrange(len(X_train)):
