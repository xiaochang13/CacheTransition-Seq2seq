...
def viterbi_transition(x_seq, emissionWeights, transitionWeights, scores, backpointers, y_cands, SOS):
    N = len(x_seq)
    path = [None for x in xrange(N)]

    scores[0][SOS] = 0

    for i in xrange(1, N+1): #Each position is just a maximization over emission scores
...
def viterbi(x, emissionWeights, transitionWeights, firstWeights, tags):
    ...
    scores = [None for i in xrange(N)]
    backpointer = [None for i in xrange(N)]
