...
def get_amr(concept_line_reprs, category_map):
    ...
    and_structure = (category_map[0] == "AND||AND")
    node_labels = []

    for (concept_idx, map_repr) in enumerate(category_map):
        concept_l, rel_str, par_str = concept_line_reprs[concept_idx]
        assert concept_l != "NONE"

        if "NE" in concept_l and "NEG" not in concept_l:
            if concept_l == "NE":
                root_repr = "person"
            else:
    ...
    try:
        ...
        print concept_line_reprs, map_repr
        sys.exit(1)
