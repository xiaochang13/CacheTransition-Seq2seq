...
def evaluate(sess, valid_graph, devDataStream, options=None, suffix=''):
    ...
    for batch_index in xrange(devDataStream.get_num_batch()): # for each batch
        ...
        if valid_graph.mode == 'evaluate':
            accu_value, loss_value, _ = valid_graph.run_ce_training(sess, cur_batch, options, only_eval=True)
            dev_loss += loss_value
            dev_right += accu_value
            dev_total += np.sum(cur_batch.action_length)
...
def main(_):
    ...
    if FLAGS.infile_format == 'fof':
        trainset = soft_NP2P_data_stream.read_generation_datasets_from_fof(FLAGS.train_path, isLower=FLAGS.isLower)
    else:
        trainset = soft_NP2P_data_stream.read_all_GenerationDatasets(FLAGS.train_path, isLower=FLAGS.isLower)
    print('Number of training samples: {}'.format(len(trainset)))

    print('Loading test set.')
    if FLAGS.infile_format == 'fof':
        testset = soft_NP2P_data_stream.read_generation_datasets_from_fof(FLAGS.test_path, isLower=FLAGS.isLower)
    else:
        testset = soft_NP2P_data_stream.read_all_GenerationDatasets(FLAGS.test_path, isLower=FLAGS.isLower)
    print('Number of test samples: {}'.format(len(testset)))

    FLAGS.feat_num = len(trainset[0][3][0])
    print('feature num: {}'.format(FLAGS.feat_num))
    ...
    if os.path.exists(best_path + ".index"):
        ...
        feat_vocab = Vocab(path_prefix + ".feat_vocab", fileformat='txt2')
        ...
        (allWords, allChars, allPOSs, allFeats, allActions) = soft_NP2P_data_stream.collect_vocabs(trainset)
        ...
        print('Number of allFeats: {}'.format(len(allFeats)))
        print('Number of allActions: {}'.format(len(allActions)))
        ...
        feat_vocab = Vocab(voc=allFeats, dim=FLAGS.feat_dim, fileformat='build')
        feat_vocab.dump_to_txt2(path_prefix + ".feat_vocab")
        action_vocab = Vocab(voc=allActions, dim=FLAGS.action_dim, fileformat='build')
        action_vocab.dump_to_txt2(path_prefix + ".action_vocab")

    print('word vocab size {}'.format(word_vocab.vocab_size))
    sys.stdout.flush()

    print('Build DataStream ... ')
    trainDataStream = soft_NP2P_data_stream.DataStream(trainset, word_vocab, char_vocab, POS_vocab, feat_vocab, action_vocab,
            options=FLAGS, isShuffle=True, isLoop=True, isSort=True)

    devDataStream = soft_NP2P_data_stream.DataStream(testset, word_vocab, char_vocab, POS_vocab, feat_vocab, action_vocab,
