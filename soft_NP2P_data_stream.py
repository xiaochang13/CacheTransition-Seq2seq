...
def read_all_GenerationDatasets(inpath, isLower=True, dep_path=None, token_path=None):
    ...
    for instance in dataset:
        ...
        concepts = instance['concepts'].strip().split()
        categories = instance['annotation']['categories'].split()
        map_infos = instance['annotation']['mapinfo'].split("_#_")
        assert len(concepts) == len(categories) and len(concepts) == len(map_infos)
    ...
    if dep_path:
        ...
        trees = oracle.ioutil.loadDependency(dep_path, all_orig_tokens, True)
...
def read_Testset(indir, isLower=True):
...
class Batch(object):
    def __init__(self, instances, options, word_vocab=None, char_vocab=None, POS_vocab=None, feat_vocab=None, action_vocab=None):
        ...
        for (input_sent, concepts_idx, cid2wid, feats_idx, actions_idx, action2cid, action2wid) in instances:
            self.input_length.append(input_sent.get_length())
            self.concept_length.append(len(concepts_idx))
        ...
        end_id = action_vocab.getIndex('</s>')
        self.action_inp = []
        self.action_ref = []
        self.feats = []
        for (input_sent, concepts_idx, cid2wid, feats_idx, actions_idx, action2cid, action2wid) in instances:
        ...
        for (input_sent, concepts_idx, cid2wid, feats_idx, actions_idx, action2cid, action2wid) in instances:
            self.input_word.append(input_sent.word_idx_seq)
            self.concept_word.append(concepts_idx)
        ...
        if options.with_lemma:
            self.input_lemma = []
            for (input_sent, concepts_idx, cid2wid, feats_idx, actions_idx, action2cid, action2wid) in instances:
                self.input_lemma.append(input_sent.lemma_idx_seq)
        ...
        if options.with_POS:
            ...
            for (input_sent, concepts_idx, cid2wid, feats_idx, actions_idx, action2cid, action2wid) in instances:
                self.input_POS.append(input_sent.POS_idx_seq)
...
# read_all_GenerationDatasets(sys.argv[1], False, sys.argv[2], sys.argv[3])
