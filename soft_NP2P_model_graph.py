...
class ModelGraph(object):
    def __init__(self, word_vocab=None, char_vocab=None, POS_vocab=None, feat_vocab=None, action_vocab=None,
        ...
        with variable_scope.variable_scope("generator"):
            # create generator
            self.generator = soft_generator_utils.AttnGen(self, options, action_vocab, feat_vocab)
            # calculate encoder_features
            with variable_scope.variable_scope("input_feats"):
                self.input_features = self.generator.calculate_encoder_features(self.input_hiddens,
                        self.input_hidden_dim)
            with variable_scope.variable_scope("concept_feats"):
                self.concept_features = self.generator.calculate_encoder_features(self.concept_hiddens,
            ...
            if mode == 'decode':
                ...
                (self.state_t, self.context_input_t, self.context_concept_t, self.ouput_t,
                    self.topk_log_probs, self.topk_ids, self.greedy_prediction, self.sample_prediction) = self.generator.decode_mode(
                        self.init_decoder_state, self.context_input_t_1, self.context_concept_t_1, self.actionidx_t, self.featidx_t,
                        self.input_hiddens, self.input_features, self.input_mask, self.concept_hiddens, self.concept_features, self.concept_mask)
                ...
                _, _, self.greedy_words = self.generator.train_mode(
                        self.input_hidden_dim, self.input_hiddens, self.input_features, self.input_mask,
                        self.concept_hidden_dim, self.concept_hiddens, self.concept_features, self.concept_mask,
                        self.init_decoder_state, self.action_inp, self.action_ref, self.feats, gen_loss_mask, mode_gen='greedy')
                # not buiding training op for this mode
                return
                ...
                self.accu, self.loss, self.sampled_words = self.generator.train_mode(
                        self.input_hidden_dim, self.input_hiddens, self.input_features, self.input_mask,
                        self.concept_hidden_dim, self.concept_hiddens, self.concept_features, self.concept_mask,
                        self.init_decoder_state, self.action_inp, self.action_ref, self.feats, gen_loss_mask, mode_gen='ce_loss')
                if mode == 'evaluate': return # not buiding training op for evaluation
            elif mode == 'topk':
                self.accu, self.sampled_words = self.generator.train_mode(
                        self.input_hidden_dim, self.input_hiddens, self.input_features, self.input_mask,
                        self.concept_hidden_dim, self.concept_hiddens, self.concept_features, self.concept_mask,
                        self.init_decoder_state, self.action_inp, self.action_ref, self.feats, gen_loss_mask, mode_gen='topk')
                return
            elif mode == 'rl_train':
                assert False, 'not in use'
                _, self.loss, _ = self.generator.train_mode(
                        self.input_hidden_dim, self.input_hiddens, self.input_features, self.input_mask,
                        self.concept_hidden_dim, self.concept_hiddens, self.concept_features, self.concept_mask,
                        self.init_decoder_state, self.action_inp, self.action_ref, self.feats, gen_loss_mask, mode_gen='rl_loss')
                ...
                _, _, self.sampled_words = self.generator.train_mode(
                        self.input_hidden_dim, self.input_hiddens, self.input_features, self.input_mask,
                        self.concept_hidden_dim, self.concept_hiddens, self.concept_features, self.concept_mask,
                        self.init_decoder_state, self.action_inp, self.action_ref, self.feats, gen_loss_mask, mode_gen='sample')
    ...
    def run_topk(self, sess, batch, options):
        feed_dict = self.run_encoder(sess, batch, options, only_feed_dict=True) # reuse this function to construct feed_dict
        ...
        return sess.run([self.accu, self.sampled_words], feed_dict)
    ...
    def run_encoder(self, sess, batch, options, only_feed_dict=False):
        ...
        return sess.run([self.init_decoder_state,
            self.input_hiddens, self.input_features, self.input_mask,
            self.concept_hiddens, self.concept_features, self.concept_mask], feed_dict)
